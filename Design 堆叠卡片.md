...
## 17. P7 计划草案（2026-08-23，待用户确认后开工）
...
## 18. C1 + B 反馈感第一批（2026-08-23）

**C1 第四卡「标化题库」骨架**：04 卡 comingSoon 占位、鼠尾草绿渐变兜底
（board.jpg 用户出图后放入即生效）、四卡横滑/键盘循环实测；卡槽映射泛化
（0→前 / 1→右 / 末→左 / 其余→中层）。

**B2 完卷横幅**：交卷让某卷跨到做满 → 成绩页流光横幅（充电条同款纹理），
快照对比判定、不落盘不重复弹；锁定用户不可能报出扩展卷名（reachable 求交 +
现库零混卷）。**审查 A1 采纳**：横幅算在落盘前，「跳过本场统计」那条路不写
记录——加小字「计入统计后，卷面进度墙上这一格就会点亮」，把条件说破，
不再断言既成事实（有绑定断言钉住）。

**B3 重练闭环**：重练场成绩页「上次错的这 N 道，这次对了 M 道」（审查 N4
改成对比式，光复述大比分没有信息量）；origin 标记只在 start() 写、
stashGrillReceipt 清（N3），不进 SessionRecord / XLSX。

**B5 复烤区回执**：三种来源三种措辞（复烤/重练/知识点），state 不落盘、
开新场即清；诊断开考也清（N2——诊断也是「新的上一场」）。

**审查 A2 采纳**：四卡后第三层卡命中带 375px 只剩 11px（三卡时代无此题，
键盘/滑动也要两步而非自陈的一步）——slot-back-y -9%→-12%、viewport
padding-top 18→30px，实测命中带 22px。N1 comingSoon 卡的读屏标签改念
「即将开放」（不再骗「展开配置」）；N5 暗色下横幅标题渐变停靠色抬到
#00c8c2（原 3.4:1）；N7 三处陈旧注释。备案：N6 board.jpg 404（等用户图）、
B2 仪式感优先于落盘时序（小字已把话说破）。

## 18. 考试日历与倒计时（设计稿，2026-08-30，待用户确认）
...
### 17.1 C1+B 第一批落地与审查裁决（2026-08-30）

上线：04「标化题库」骨架卡（用户出图已接，档案馆绿）；B2 完卷横幅；
B3 重练闭环「上次错的这 N 道，这次对了 M 道」；B5 复烤区回执（三种来源
三种措辞）。测试 132 → 146。

审查 PASS-WITH-FIXES 裁决：
- A1 采纳：横幅在落盘前渲染（仪式感不等按钮），文案改条件式真话
  「完卷 · 计入统计后这一套就做满」——点「跳过本场统计」的人看到的
  也不是假话；测试钉住旧句不得回潮
- A2 采纳：第三层卡触屏命中带 11px → 净露 6% 卡高（-12%/30px），
  「一步可达」的自陈被审查证伪（环上距 2 格需按两次方向键）
- N1–N5/N7 全部采纳（读屏 comingSoon 分支、Diagnostic 清回执、
  origin 离开清、B3 对比式文案、横幅暗色端点色、陈旧注释）；
  N6 已由用户补图解决；N8 补绑定断言
- 备案：B3/B5 对重练场双报维持（不同视图不同时刻）；「做满」含错题
  维持（与卷墙同判据，测试钉住）
