...
- 卡位：04 卡，zones.ts 数据驱动加一项即可（deck 已支持任意张数，手机横滑
  需实测四卡手感）；先上 comingSoon 骨架（P1 时 Grill 的先例），
  封面图 prompt 补一段（风格前缀沿用 §8）
- 形态猜想：以锁卷机制为底座的整卷限时训练（与 A 复用）
- **内容待用户定**：装哪些考试？vault 里的候补有 ESAT（空）/ STEP（179 题，
  长答题居多，现引擎只能判 MCQ）/ Euclid（594）/ Hypatia（119）/ BMO（空）；
  或是全新库（SAT/ACT？）。题型必须是单选 MCQ 才能直接进现引擎
- 开放策略：骨架卡先行 → 内容管线就绪后按锁卷形态开放

### 待用户拍板的三个问题
1. A 的「然后也作为——」后半句：也作为什么？
2. C 训练场装什么内容（考试名单 + 是否纯 MCQ）？
3. B 清单按建议优先级（2/3/5 先行）可以吗，还是你另有排序？
